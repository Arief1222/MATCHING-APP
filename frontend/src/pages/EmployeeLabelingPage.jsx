...
const EmployeeLabelingPage = () => {
  ...
  const submitLabeling = async (itemId, label) => {
    try {
      setSubmitting(true);
      console.log(`Submitting label for item ${itemId}:`, label);

      const response = await fetch('http://127.0.0.1:8001/submit-my-labeling/', {
        method: 'POST',
        headers: getApiHeaders(),
        body: JSON.stringify({
          labeling_id: itemId,
      ...
      if (response.ok) {
        showMessage(`Data berhasil dilabeling sebagai ${label}`, 'success');
        ...
        showMessage(data.error || 'Gagal menyimpan label', 'error');
      }
    } catch (error) {
  ...
  const handleSubmit = async (itemId) => {
    ...
    if (!selectedLabel) {
      showMessage('Pilih salah satu opsi terlebih dahulu', 'error');
      return;
    }
        ...
        <div className="p-6 border-b bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-t-lg">
          ...
          {message && (
            <div className={`mt-4 p-4 rounded-lg flex items-center ${
              messageType === 'success' ? 'bg-green-500/20 border border-green-400/30' :
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {/* <div className="bg-white p-4 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-gray-900">{summary.total_assigned || 0}</div>
            ...
            {/* <div className="bg-white p-4 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-gray-900">{summary.remaining || 0}</div>
          ...
          {labelingData.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <div className="mb-4 text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
                <strong>Petunjuk:</strong> Pilih salah satu opsi (MATCH atau UNMATCH) untuk setiap pasangan data, 
                kemudian klik tombol Submit untuk menyimpan hasil labeling.
              </div>
                  ...
                  {labelingData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4 text-sm text-gray-900">
                        <label className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-green-50 cursor-pointer">
                          <input
                            type="radio"
                            name={`data_${item.id}`}
                            value="MATCH"
                      ...
                      <td className="px-4 py-4 text-sm text-gray-900">
                        <label className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-red-50 cursor-pointer">
                          <input
                            type="radio"
                            name={`data_${item.id}`}
                            value="UNMATCH"
                            ...
                            onChange={(e) => handleLabelSelection(item.id, e.target.value)}
                            className="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 mt-1"
                          />
                          <div>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-sm">
                        <button
                          ...
                          className={`px-4 py-2 rounded font-medium text-sm flex items-center space-x-2 ${
          ...
          {labelingData.length > 0 && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center text-blue-800 text-sm">
                ...
                <span>
                  Pastikan Anda telah memilih opsi yang tepat sebelum menekan tombol Submit. 
